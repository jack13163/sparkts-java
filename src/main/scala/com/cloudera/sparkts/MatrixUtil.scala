...
private object MatrixUtil {
   ...
   */
  private def fromBreeze(breeze: Matrix[Double]): SM = {
    breeze match {
   ...
   * @return a spark-mllib Matrix instance
   */
  private def toBreeze(sparkMatrix: SM): Matrix[Double] = {
    sparkMatrix match {
      case dm: SDM =>
        if (!dm.isTransposed) {
   ...
   */
  private def fromBreeze(breezeVector: Vector[Double]): SV = {
    breezeVector match {
   ...
   */
  private def toBreeze(sparkVector: SV): Vector[Double] = {
    sparkVector match {
      case v: SDV =>
  ...
  private implicit def mSparkToBreeze(sparkMatrix: SM): Matrix[Double] =
    toBreeze(sparkMatrix)

  private implicit def dmSparkToBreeze(sparkMatrix: SDM): DenseMatrix[Double] =
    toBreeze(sparkMatrix).asInstanceOf[DenseMatrix[Double]]

  private implicit def mBreezeToSpark(breezeMatrix: Matrix[Double]): SM =
    fromBreeze(breezeMatrix)

  private implicit def dmBreezeToSpark(breezeMatrix: DenseMatrix[Double]): SDM =
    fromBreeze(breezeMatrix).asInstanceOf[SDM]

  private implicit def vSparkToBreeze(sparkVector: SV): Vector[Double] =
    toBreeze(sparkVector)

  private implicit def dvSparkToBreeze(sparkVector: SDV): DenseVector[Double] =
    toBreeze(sparkVector).asInstanceOf[DenseVector[Double]]

  private implicit def vBreezeToSpark(breezeVector: Vector[Double]): SV =
    fromBreeze(breezeVector)

  private implicit def dvBreezeToSpark(breezeVector: DenseVector[Double]): SDV =
    fromBreeze(breezeVector).asInstanceOf[SDV]

  private implicit def fvtovBreezeToSpark(f: (Vector[Double]) => Vector[Double])
    : (SV) => SV = {
    v: SV => f(v)
  }
